"""Extract highlights, typed notes, and ink strokes from reMarkable bundles.

All rmscene-specific parsing lives here. Zotero-only users don't need
the optional ``rmscene`` dependency — see ``distillate.renderer`` for
the generic PDF annotation rendering path.
"""
...
# rmscene is lazy-imported — install via ``pip install "distillate[remarkable]"``.
...
# Highlight annotation style (matches distillate.renderer defaults)
_HIGHLIGHT_COLOR = (1.0, 0.92, 0.3)  # soft yellow
_HIGHLIGHT_OPACITY = 0.35
_HIGHLIGHT_TRIM = 0.20
    ...
    for page_idx, glyph_list in highlights_by_page.items():
        ...
        for text, rm_y in glyph_list:
            ...
            if len(groups) > 1 and rm_y is not None:
                expected_frac = (rm_y / _SEARCH_RM_HEIGHT) * _RM_TO_PDF_SCALE
                groups = [min(
                    groups,
...
def extract_zotero_highlights(
    ...
    """Extract highlight positions from an .rmscene bundle in Zotero format.

    Used for back-propagating reMarkable-made highlights to Zotero.
...
# ---------------------------------------------------------------------------
# Internal helpers — glyphs, quads, text cleanup
# ---------------------------------------------------------------------------
...
def extract_ink_strokes(zip_path: Path) -> Dict[int, list]:
...
def _is_paper_pro_coords(ink_by_page: Dict[int, list]) -> bool:
...
def _render_ink_on_pdf(doc, ink_by_page: Dict[int, list]) -> int:
